...
bool TrackballCameraController::update(float elapsedTime)
{
  ...
  if (glfwGetKey(m_pWindow, GLFW_KEY_LEFT_SHIFT)) {
    const auto panLeft = 0.01f * float(cursorDelta.x);
    const auto panUp = 0.01f * float(cursorDelta.y);
    const auto hasMoved = panLeft || panUp;

    if (!hasMoved) {
      return false;
    }

    m_camera.moveLocal(panLeft, panUp, 0.f);
    return true;
  }

  if (glfwGetKey(m_pWindow, GLFW_KEY_LEFT_CONTROL) ||
      glfwGetKey(m_pWindow, GLFW_KEY_RIGHT_CONTROL)) {
    float mouseX = 0.01f * float(cursorDelta.x);
    if (mouseX == 0.0f) {
      return false;
    }

    const glm::vec3 viewVector = m_camera.center() - m_camera.eye();
    const float dist = glm::length(viewVector);

    if (mouseX > 0.f) {
      mouseX = glm::min(mouseX, dist - 0.1f);
    }

    const glm::vec3 eye = m_camera.eye() + mouseX * viewVector / dist;
    m_camera = Camera(eye, m_camera.center(), m_worldUpAxis);
  ...
  const auto latRotationMatrix =
      glm::rotate(mat4(1), latitudeAngle, m_worldUpAxis);
  const auto latLongRotationMatrix =
      glm::rotate(latRotationMatrix, longitudeAngle, m_camera.left());

  const glm::vec3 finalDepthAxis =
      vec3(latLongRotationMatrix * vec4(depthAxis, 0));

  const glm::vec3 eye = m_camera.center() + finalDepthAxis;
  m_camera = Camera(eye, m_camera.center(), m_worldUpAxis);
